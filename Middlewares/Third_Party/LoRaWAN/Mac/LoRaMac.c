...
LoRaMacStatus_t SendFrameOnChannel( uint8_t channel )
{
    ...
    LoRaMacGetFCntUp( &fCntUp );
		...
		PRINTF("[%04ld][%s] seqTX = %d, CH=%d, DR=%d, TXPow=%d, buflen = %d\r\n",__LINE__,  __FUNCTION__, fCntUp, (int)channel, (int)txConfig.Datarate,  (int)txConfig.TxPower, MacCtx.PktBufferLen);
    // Send now
